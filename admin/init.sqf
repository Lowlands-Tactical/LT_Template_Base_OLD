...
if ( missionNamespace getVariable ["ace_common", false]) then {
	...
	// _action = ["lt_main_ghost", "Hide player", "", {[player,true] call ALIVE_fnc_adminGhost}, { !isObjectHidden player and getPlayerUID player in _staff }] call ace_interact_menu_fnc_createAction;
	// [player, 1, ["ACE_SelfActions", "lt_main"], _action] call ace_interact_menu_fnc_addActionToObject;

	// _action = ["lt_main_unghost", "Unhide player", "", {[player,false] call ALIVE_fnc_adminGhost}, { isObjectHidden player and getPlayerUID player in _staff }] call ace_interact_menu_fnc_createAction;
	// [player, 1, ["ACE_SelfActions", "lt_main"], _action] call ace_interact_menu_fnc_addActionToObject;
