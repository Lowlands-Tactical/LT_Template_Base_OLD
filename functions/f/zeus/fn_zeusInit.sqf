...
diag_log format ["LT template DEBUG: Zeusinit _this: %1", _this];
...
if !(isNull (getAssignedCuratorLogic _unit)) exitWith {
	diag_log format ["DEBUG (lt_template_base\functions\f\zeus\fn_zeusInit.sqf): ZEUS already assigned to %1.",_unit];
	systemchat format ["DEBUG (lt_template_base\functions\f\zeus\fn_zeusInit.sqf): ZEUS already assigned to %1.",_unit];
};

// Exit if the unit is not a player
if !(isPlayer _unit) exitWith {
	diag_log format ["DEBUG (lt_template_base\functions\f\zeus\fn_zeusInit.sqf): Did not assign ZEUS to %1 because it is not under control of a player.",_unit];
	systemchat format ["DEBUG (lt_template_base\functions\f\zeus\fn_zeusInit.sqf): Did not assign ZEUS to %1 because it is not under control of a player.",_unit];
};
...
/* NOT NEEDED IN TEMPLATE BECAUSE OF SL_SKILL
