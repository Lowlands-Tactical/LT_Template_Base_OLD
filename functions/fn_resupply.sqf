...
private _spawnedData=[_aircraftSide,_startPosition,0,_startPosition getDir (_waypoints select 0),false,true,_aircraftClass] call LT_fnc_resupplycreateveh;
{
	_x allowFleeing 0;
...
for [{_idx=0},{_idx < count _waypoints},{_idx=_idx+1}] do {
	...
	if (_idx==_dropWp) then {
		[(_spawnedData select 1),_idx+1] setWaypointStatements ["true", "n0 = [this, '"+_side+"'] spawn LT_fnc_actualdrop;"];
	};
