...
diag_log format ["*-* test *-* lt_camo_var: %1", lt_camo_var];
...
//diag_log format ["*-* LT Test *-* camo: %1 - vest: %2 - pack: %3 - helm: %4", _camo, _vest, _pack, _helm];
//diag_log format ["*-* LT Test *-* variable: %1", player getVariable ["LT_camo_exclude", 0]];

	diag_log format ["*-* LT Test *-* is none: %1 unit is excluded %2", _camo != "None", player getVariable ["LT_camo_exclude", 0] == 0];
