...
if (_exclude == 0) then {
	if (typename _camo == "ARRAY") then {
		...
		_CamoUniform 	= [player, selectRandom _camo] call lt_fnc_changeUniform;
		_CamoVest 		= [player, selectRandom _vest] call lt_fnc_changeVest;
		_CamoBackpack = [player, selectRandom _pack] call lt_fnc_changeBackpack;
		removeHeadgear player;
		player addHeadgear selectRandom _helm;
