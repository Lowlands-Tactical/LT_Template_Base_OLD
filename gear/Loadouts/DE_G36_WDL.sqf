// filename		"DE_G36_Camo.sqf"
// loadout name "DE G36 Camo"

lt_template_scope1 = "CUP_optic_HoloBlack";
lt_template_attach1 = "acc_flashlight";
lt_template_attachments    	 = 	 [lt_template_attach1, lt_template_scope1];

// Assistant Automatic Rifleman, Medium MG assistant, UAV Operator, Rifleman
lt_template_rifle = "CUP_arifle_G36A_camo";
lt_template_riflemag = "CUP_30Rnd_556x45_G36";
lt_template_riflemag_t = "CUP_30Rnd_TE1_Red_Tracer_556x45_G36";

//Medic, Rifleman At
lt_template_carbine = "CUP_arifle_G36C_camo";
lt_template_carbinemag = "CUP_30Rnd_556x45_G36";
lt_template_carbinemag_tr = "CUP_30Rnd_TE1_Red_Tracer_556x45_G36";

// Pilots, Vehicle crew, Submachinegunner
lt_template_smg = "CUP_smg_EVO_MRad_Flashlight";
lt_template_smgmag = "CUP_30Rnd_9x19_EVO";
lt_template_smgmag_tr = "CUP_30Rnd_9x19_EVO";

// Fuck you
lt_template_AR = "CUP_arifle_MG36_camo_holo";
lt_template_ARmag = "CUP_100Rnd_556x45_BetaCMag";
lt_template_ARmag_tr = "CUP_100Rnd_TE1_Red_Tracer_556x45_BetaCMag";

// Medium MG Gunner
lt_template_MMG = "CUP_lmg_M60E4";
lt_template_MMGmag = "CUP_100Rnd_TE4_LRT4_White_Tracer_762x51_Belt_M";
lt_template_MMGmag_tr = "CUP_100Rnd_TE4_LRT4_Red_Tracer_762x51_Belt_M";

// Designated Marksman
lt_template_DMrifle = "CUP_srifle_G22_des";
lt_template_DMriflemag = "CUP_5Rnd_762x67_G22";

lt_template_RAT = "CUP_launch_MAAWS_Scope";
lt_template_RATmag1 = "CUP_MAAWS_HEAT_M";
lt_template_RATmag2 = "CUP_MAAWS_HEAT_M";

lt_template_MAT	= "CUP_launch_M47";
lt_template_MATmag1		= "CUP_Dragon_EP1_M";
lt_template_MATmag2		= "CUP_Dragon_EP1_M";

lt_template_glrifle = "CUP_arifle_G36A_camo";
lt_template_glriflemag = "CUP_30Rnd_556x45_G36";
lt_template_glriflemag_tr = "CUP_30Rnd_TE1_Red_Tracer_556x45_G36";
lt_template_glmag = "HandGrenade";
lt_template_glsmokewhite = "SmokeShell";
lt_template_glsmokegreen = "SmokeShellRed";
lt_template_glsmokered = "SmokeShellGreen";
lt_template_glflarewhite = "CUP_30Rnd_556x45_G36";
lt_template_glflarered = "ACE_HandFlare_Red";
lt_template_glflareyellow = "ACE_HandFlare_Yellow";
lt_template_glflaregreen = "ACE_HandFlare_Green";

// Medium SAM Gunner, Medium SAM Gunner Assistant (alleen magazijnen)
lt_template_SAM = "CUP_launch_FIM92Stinger";
lt_template_SAMmag = "CUP_Stinger_M";

// Heavy AT Gunner, Heavy AT Gunner Assistant (alleen magazijnen)
lt_template_HAT    	 = 	 "CUP_launch_Javelin";
lt_template_HATmag1        	 = 	 "CUP_Javelin_M";
lt_template_HATmag2        	 = 	 "CUP_Javelin_M";

// Sniper
lt_template_SNrifle        	 = 	 "CUP_srifle_M107_Base";
lt_template_SNrifleMag     	 = 	 "CUP_10Rnd_127x99_M107";


// Static weapons
lt_template_baghmgg = "CUP_B_M2_Gun_Bag";
lt_template_baghmgag = "CUP_B_M2_Tripod_Bag";
lt_template_baghatg = "CUP_B_Tow_Gun_Bag";
lt_template_baghatag = "CUP_B_TOW_Tripod_Bag";
lt_template_bagmtrg = "CUP_B_M252_Gun_Bag";
lt_template_bagmtrag = "CUP_B_M252_Bipod_Bag";