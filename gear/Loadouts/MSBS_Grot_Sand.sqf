// filename 	"MSBS_Grot_Sand.sqf"
// loadout name "MSBS Grot Sand"

// Attachments Primary weapon
lt_template_attach1        	 = 	 "acc_pointer_IR"; // ir laser
lt_template_attach2        	 = 	 "acc_flashlight"; // flashlight
lt_template_silencer1      	 = 	 ""; // rifles
lt_template_silencerSMG      	 = 	 ""; // MG
lt_template_scope1         	 = 	 "optic_ico_01_sand_f"; // normal scope
lt_template_scopeSNP         = 	 "optic_SOS"; // Sniper Scope
lt_template_scopeDM          = 	 ""; // DM Scope
lt_template_bipod1         	 = 	 "bipod_01_F_blk"; // MG
lt_template_bipod2         	 = 	 "";
lt_template_attachments    	 = 	 [lt_template_attach1, lt_template_scope1];

// Handgun attachements
lt_template_hg_silencer1         	 = 	 "";
lt_template_hg_scope1      	 = 	 "";
lt_template_hg_attachments         	 = 	 [];

// Assistant Automatic Rifleman, Medium MG assistant, UAV Operator, Rifleman
lt_template_rifle          	 = 	 "arifle_MSBS65_sand_F";
lt_template_riflemag       	 = 	 "30Rnd_65x39_caseless_msbs_mag";
lt_template_riflemag_t     	 = 	 "30Rnd_65x39_caseless_msbs_mag_Tracer";

//Medic, Rifleman At
lt_template_carbine        	 = 	 "arifle_MSBS65_sand_F";
lt_template_carbinemag     	 = 	 "30Rnd_65x39_caseless_msbs_mag";
lt_template_carbinemag_tr    = 	 "30Rnd_65x39_caseless_msbs_mag_Tracer";

// Pilots, Vehicle crew, Submachinegunner
lt_template_smg    	 = 	 "SMG_01_F";
lt_template_smgmag         	 = 	 "30Rnd_45ACP_Mag_SMG_01";
lt_template_smgmag_tr      	 = 	 "30Rnd_45ACP_Mag_SMG_01_Tracer_Red";

// Commander, Squadleader, Fireteam leader, Spotter, Grenadier
lt_template_glrifle        	 = 	 "arifle_MSBS65_GL_sand_F";
lt_template_glriflemag     	 = 	 "30Rnd_65x39_caseless_msbs_mag";
lt_template_glriflemag_tr    = 	 "30Rnd_65x39_caseless_msbs_mag_Tracer";
lt_template_glmag          	         = 	 "1Rnd_HE_Grenade_shell";
lt_template_glsmokewhite           	 = 	 "1Rnd_Smoke_Grenade_shell";
lt_template_glsmokegreen           	 = 	 "1Rnd_SmokeGreen_Grenade_shell";
lt_template_glsmokered     	         = 	 "1Rnd_SmokeRed_Grenade_shell";
lt_template_glflarewhite           	 = 	 "UGL_FlareWhite_F";
lt_template_glflarered     	         = 	 "UGL_FlareRed_F";
lt_template_glflareyellow          	 = 	 "UGL_FlareYellow_F";
lt_template_glflaregreen           	 = 	 "UGL_FlareGreen_F";

// Commander, Squadleader, Automatic Rifleman, Medium Machinegunner, Designated Marksman, Sniper
lt_template_pistol         	 = 	 "hgun_Pistol_heavy_01_F";
lt_template_pistolmag      	 = 	 "11Rnd_45ACP_Mag";

// Automatic Rifleman
lt_template_AR     	 = 	 "LMG_Mk200_F";
lt_template_ARmag          	 = 	 "200Rnd_65x39_cased_Box_Red";
lt_template_ARmag_tr       	 = 	 "200Rnd_65x39_cased_Box_Red";

// Medium MG Gunner
lt_template_MMG    	 = 	 "MMG_02_sand_F";
lt_template_MMGmag         	 = 	 "130Rnd_338_Mag";
lt_template_MMGmag_tr      	 = 	 "130Rnd_338_Mag";

// Designated Marksman
lt_template_DMrifle        	 = 	 "arifle_MSBS65_Mark_sand_F";
lt_template_DMriflemag     	 = 	 "30Rnd_65x39_caseless_mag";

// Rifleman AT
lt_template_RAT = "launch_NLAW_F";

// Medium AT Gunner, Medium AT Gunner Assistant (alleen magazijnen)
lt_template_MAT    	 = 	 "launch_B_Titan_short_F";
lt_template_MATmag1        	 = 	 "Titan_AT";
lt_template_MATmag2        	 = 	 "Titan_AT";

// Medium SAM Gunner, Medium SAM Gunner Assistant (alleen magazijnen)
lt_template_SAM    	 = 	 "launch_B_Titan_F";
lt_template_SAMmag         	 = 	 "Titan_AA";

// Heavy AT Gunner, Heavy AT Gunner Assistant (alleen magazijnen
lt_template_HAT    	 = 	 "launch_B_Titan_short_F";
lt_template_HATmag1        	 = 	 "Titan_AT";
lt_template_HATmag2        	 = 	 "Titan_AT";

// Sniper
lt_template_SNrifle             =      "srifle_LRR_F";
lt_template_SNrifleMag          =      "7Rnd_408_Mag";

// Grenades
lt_template_grenade        	 = 	 "CUP_HandGrenade_M67";
lt_template_Mgrenade       	 = 	 "MiniGrenade";

// Static weapons
lt_template_baghmgg        	 = 	 "B_HMG_01_high_weapon_F";
lt_template_baghmgag       	 = 	 "B_HMG_01_support_high_F";

lt_template_baghatg        	 = 	 "B_AT_01_weapon_F";
lt_template_baghatag       	 = 	 "B_HMG_01_support_F";

lt_template_bagmtrg        	 = 	 "B_Mortar_01_weapon_F";
lt_template_bagmtrag       	 = 	 "B_Mortar_01_support_F";
