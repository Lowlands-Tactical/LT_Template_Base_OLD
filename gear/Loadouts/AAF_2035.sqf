// filename 	"AAF2035.sqf"
// loadout name "AAF 2035"

// Attachments Primary weapon
lt_template_attach1        	 = 	 "acc_pointer_IR"; // ir laser
lt_template_attach2        	 = 	 "acc_flashlight"; // flashlight
lt_template_silencer1      	 = 	 ""; // rifles
lt_template_silencer2      	 = 	 ""; // MG
lt_template_scope1         	 = 	 "optic_MRCO"; // normal scope
lt_template_scope2         	 = 	 "optic_LRPS"; // scope sniper
lt_template_scope3         	 = 	 "";
lt_template_bipod1         	 = 	 "bipod_03_F_blk"; // MG
lt_template_bipod2         	 = 	 "";
lt_template_attachments    	 = 	 [acc_pointer_IR, optic_MRCO];

// Handgun attachements
lt_template_hg_silencer1         	 = 	 "";
lt_template_hg_scope1      	 = 	 "";
lt_template_hg_attachments         	 = 	 [];

// Assistant Automatic Rifleman, Medium MG assistant, UAV Operator, Rifleman
lt_template_rifle          	 = 	 "arifle_Mk20C_F";
lt_template_riflemag       	 = 	 "30Rnd_556x45_Stanag";
lt_template_riflemag_t     	 = 	 "30Rnd_556x45_Stanag_Tracer_Yellow";

//Medic, Rifleman At
lt_template_carbine        	 = 	 "arifle_Mk20C_F";
lt_template_carbinemag     	 = 	 "30Rnd_556x45_Stanag";
lt_template_carbinemag_tr          	 = 	 "30Rnd_556x45_Stanag_Tracer_Yellow";

// Pilots, Vehicle crew, Submachinegunner
lt_template_smg    	 = 	 "hgun_PDW2000_F";
lt_template_smgmag         	 = 	 "30Rnd_9x21_Mag";
lt_template_smgmag_tr      	 = 	 "30Rnd_9x21_Yellow_Mag";

// Commander, Squadleader, Fireteam leader, Spotter, Grenadier
lt_template_glrifle        	 = 	 "arifle_Mk20_GL_F";
lt_template_glriflemag     	 = 	 "30Rnd_556x45_Stanag";
lt_template_glriflemag_tr          	 = 	 "30Rnd_556x45_Stanag_Tracer_Yellow";
lt_template_glmag          	 = 	 "1Rnd_HE_Grenade_shell";
lt_template_glsmokewhite           	 = 	 "1Rnd_Smoke_Grenade_shell";
lt_template_glsmokegreen           	 = 	 "1Rnd_SmokeGreen_Grenade_shell";
lt_template_glsmokered     	 = 	 "1Rnd_SmokeRed_Grenade_shell";
lt_template_glflarewhite           	 = 	 "UGL_FlareWhite_F";
lt_template_glflarered     	 = 	 "UGL_FlareRed_F";
lt_template_glflareyellow          	 = 	 "UGL_FlareYellow_F";
lt_template_glflaregreen           	 = 	 "UGL_FlareGreen_F";

// Commander, Squadleader, Automatic Rifleman, Medium Machinegunner, Designated Marksman, Sniper
lt_template_pistol         	 = 	 "hgun_ACPC2_F";
lt_template_pistolmag      	 = 	 "9Rnd_45ACP_Mag";

// Automatic Rifleman
lt_template_AR     	 = 	 "LMG_Mk200_F";
lt_template_ARmag          	 = 	 "200Rnd_65x39_cased_Box";
lt_template_ARmag_tr       	 = 	 "200Rnd_65x39_cased_Box_Tracer";

// Medium MG Gunner
lt_template_MMG    	 = 	 "MMG_01_tan_F";
lt_template_MMGmag         	 = 	 "150Rnd_93x64_Mag";
lt_template_MMGmag_tr      	 = 	 "150Rnd_93x64_Mag";

// Designated Marksman
lt_template_DMrifle        	 = 	 "srifle_EBR_F";
lt_template_DMriflemag     	 = 	 "20Rnd_762x51_Mag";

// Rifleman AT
lt_template_RAT    	 = 	 "launch_I_Titan_short_F";
lt_template_RATmag1    	 = 	 "Titan_AT";
lt_template_RATmag2    	 = 	 "Titan_AT";

// Medium AT Gunner, Medium AT Gunner Assistant (alleen magazijnen)
lt_template_MAT    	 = 	 "launch_I_Titan_short_F";
lt_template_MATmag1        	 = 	 "Titan_AT";
lt_template_MATmag2        	 = 	 "Titan_AT";

// Medium SAM Gunner, Medium SAM Gunner Assistant (alleen magazijnen)
lt_template_SAM    	 = 	 "launch_I_Titan_F";
lt_template_SAMmag         	 = 	 "Titan_AA";

// Heavy AT Gunner, Heavy AT Gunner Assistant (alleen magazijnen
lt_template_HAT    	 = 	 "launch_I_Titan_short_F";
lt_template_HATmag1        	 = 	 "Titan_AT";
lt_template_HATmag2        	 = 	 "Titan_AT";

// Sniper
lt_template_SNrifle             =      "srifle_GM6_F";
lt_template_SNrifleMag          =      "7Rnd_408_Mag"

// Grenades
lt_template_grenade        	 = 	 "CUP_HandGrenade_M67";
lt_template_Mgrenade       	 = 	 "MiniGrenade";

// Static weapons
lt_template_baghmgg        	 = 	 "I_HMG_01_high_weapon_F";
lt_template_baghmgag       	 = 	 "I_HMG_01_support_high_F";

lt_template_baghatg        	 = 	 "I_AT_01_weapon_F";
lt_template_baghatag       	 = 	 "I_HMG_01_support_F";

lt_template_bagmtrg        	 = 	 "I_Mortar_01_weapon_F";
lt_template_bagmtrag       	 = 	 "I_Mortar_01_support_F";
