...
{
	alertdebug = [] execVM "\lt_template_base\AI\GARRISON\Garrison_fncs\alerted_debug.sqf";
};
*/
...
private ["_unit","_radius","_buildingslist","_cnt","_posarray","_build","_i","_nbuild","_localbuilding","_buildpos","_group","_buildingsleft","_npos","_bposleft","_buildingBlackList"];
...
_buildingBlackList = ["CUP_A1_dum_mesto_in","CUP_A1_Dum_m2","CUP_A1_Dum_mesto2L","CUP_A1_Dum_mesto3","CUP_A1_stanice","CUP_A2_dum_m2","CUP_A2_dum_mesto2l","CUP_A2_dum_mesto3","CUP_A2_housev_1i1_dam","CUP_A2_housev_2l_dam_ruins","Land_cwa_stanice","Land_cwa_dum_m2","Land_cwa_dum_mesto2l","Land_Dum_mesto2l"];
...
nul = [_group,_groupUnits] execVM "\lt_template_base\AI\GARRISON\Garrison_fncs\leaving_check.sqf"; // leaving check;
...
if (isnil("fnc_vision_check")) then {
	fnc_vision_check = compile preProcessFileLineNumbers "\lt_template_base\AI\GARRISON\Garrison_fncs\fnc_vision_check.sqf";
	//hint "fnc_vision_check compiled";
};

if (isnil("fnc_MoveTo")) then {
	// [_unit,_pos];
	fnc_MoveTo = compile preProcessFileLineNumbers "\lt_template_base\AI\GARRISON\Garrison_fncs\fnc_MoveTo.sqf";
};

if (isnil("fnc_Patrol")) then {
	// [_leader,_patrolCenter,_patrolRadius,_bRepeating (after contact)];
	fnc_Patrol = compile preProcessFileLineNumbers "\lt_template_base\AI\GARRISON\Garrison_fncs\patrol.sqf";
};

if (isnil("fnc_Prone_Limit")) then {
	// [_leader,_patrolCenter,_patrolRadius,_bRepeating (after contact)];
	fnc_Prone_Limit = compile preProcessFileLineNumbers "\lt_template_base\AI\GARRISON\Garrison_fncs\fnc_Prone_Limit.sqf";
};

if (isnil("fnc_smartlook")) then {
	fnc_smartlook = compile preProcessFileLineNumbers "\lt_template_base\AI\GARRISON\Garrison_fncs\fnc_smartlook.sqf";
	//hint "smartlook compiled";
};

if (isnil("fnc_seek")) then {
	fnc_seek= compile preProcessFileLineNumbers "\lt_template_base\AI\GARRISON\Garrison_fncs\fnc_seek.sqf";
	//hint "seek compiled";
};

if (isnil("fnc_get_angle")) then {
	fnc_get_angle = compile preProcessFileLineNumbers "\lt_template_base\AI\GARRISON\Garrison_fncs\fnc_get_angle.sqf";
	//hint "get_angle compiled";
};

if (isnil("fnc_cansee")) then {
	fnc_cansee = compile preProcessFileLineNumbers "\lt_template_base\AI\GARRISON\Garrison_fncs\fnc_cansee.sqf";
	//hint "cansee compiled";
};

if (isnil("fnc_get_DoorPositions")) then {
	// [_house]; returns doorpositionsin model coords;
	fnc_get_DoorPositions = compile preProcessFileLineNumbers "\lt_template_base\AI\GARRISON\Garrison_fncs\fnc_get_DoorPositions.sqf";
};
...
if (isnil("fnc_willsee")) then {
	fnc_willsee = compile preProcessFileLineNumbers "\lt_template_base\AI\GARRISON\Garrison_fncs\fnc_willsee.sqf";
	//hint "willsee compiled";
};


if (isnil("fnc_willwalk")) then {
	fnc_willwalk = compile preProcessFileLineNumbers "\lt_template_base\AI\GARRISON\Garrison_fncs\fnc_willwalk.sqf";
	//hint "willwalk compiled";
};

if (isnil("fnc_indoors")) then {
	fnc_indoors = compile preProcessFileLineNumbers "\lt_template_base\AI\GARRISON\Garrison_fncs\fnc_indoors.sqf";
	//hint "indoors compiled";
};

if (isnil("fnc_sillybuild_check")) then {
	fnc_sillybuild_check = compile preProcessFileLineNumbers "\lt_template_base\AI\GARRISON\Garrison_fncs\fnc_sillybuild_check.sqf";
	//hint "sillybuild check compiled";
};
/*
if (isnil("fnc_cqc_target")) then {
	fnc_cqc_target = compile preProcessFileLineNumbers "\lt_template_base\AI\GARRISON\Garrison_fncs\fnc_cqc_target.sqf";
	//hint "cqc target compiled";
};
*/
if (isnil("CQC_AI")) then {
	// [_unit]
	CQC_AI = compile preProcessFileLineNumbers "\lt_template_base\AI\GARRISON\CQC_AI.sqf";
...
diag_log format["Garrison building blacklist: %1", _buildingBlackList];
...
if ((count _objectslist) >= 1) then
{
	for "_i" from 0 to ((count _objectslist)-1) do
	{
		...
		diag_log format["_build == %1 (%2)",typeOf _build,(typeOf _build) in _buildingBlackList];
		if (format ["%1",_build buildingPos 0] != "[0,0,0]" and !((typeOf _build) in _buildingBlackList) ) then
		{
